...
- Sends **Slack notifications** at delivery moments that need a human — elaboration sign-off, bolt complete, UAT sign-off, incident/hotfix, circuit breaker, dependency audit due — plus deterministic `.claude/settings.json` hooks for generic attention/done pings (Claude Code only; Cursor and Copilot get the event notifications alone); the webhook URL is read from an environment variable so it is never committed
